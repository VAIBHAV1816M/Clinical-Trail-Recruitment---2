...
def flatten_patient_data(patient: Patient) -> Tuple[Dict[str, Any], Optional[int]]:
    ...
    if patient.vitals:
        ...
        data.update({
            ...
            "alt": latest_vitals.alt,                # RESTORED
            "creatinine": latest_vitals.creatinine,  # RESTORED
...
def run_matching_engine(patient: Patient, trial: Trial, overrides: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    serialized_criteria = [CriterionResponse.model_validate(c) for c in trial.criteria]
    ...
    if not hard_passed:
        # Short Circuit: Don't waste compute on Gaussian math if they failed a hard gate
        explanations = generate_explanations(hard_failures, [])
        return {
            "match_percentage": 0.0,
            "verdict": "REJECTED",
            "eligible": False,
    ...
    # 5. Determine Verdict
    verdict = determine_verdict(match_percentage, hard_passed=True)
    eligible = (verdict in ["APPROVED", "NEEDS_REVIEW"])
    
    # 6. Generate Explainability Snapshot
    explanations = generate_explanations([], soft_contributions)
