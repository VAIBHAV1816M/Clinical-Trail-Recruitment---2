...
export function PatientsPage({ onSelectPatient }) {
  const { patients, registerPatient, showToast } = useApp();
  ...
  const handleSimulateBatchUpload = () => {
    showToast('Batch Upload Success', 'Simulated ingestion: 25 rows processed, 22 inserted, 3 duplicate flags.', 'success');
        ...
        <Modal
          ...
          footer={
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem', width: '100%' }}>
              ...
              <button className="btn btn-primary" onClick={handleSimulateBatchUpload}>Ingest Dataset</button>
            </div>
          }
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ border: '2px dashed var(--slate-300)', padding: '2.5rem 1rem', borderRadius: 'var(--radius-lg)', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem' }}>
              <FileSpreadsheet size={36} color="#0284c7" />
              <div style={{ fontWeight: 600, color: 'var(--slate-800)' }}>
                Drag & Drop Patient Roster (.xlsx or .xls)
              </div>
              <div style={{ fontSize: '0.78rem', color: 'var(--slate-500)' }}>
                Supports automatic duplicate detection and vitals snapshot parsing.
              </div>
            </div>
          </div>
        </Modal>
